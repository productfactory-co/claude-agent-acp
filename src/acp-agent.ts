...
// v0.62.0 (Apache-2.0); upstream LICENSE retained. See FORK.md.
import {
  agent as acpAgent,
  AgentContext,
  AuthenticateRequest,
...
export function streamEventToAcpNotifications(
  ...
  // completes. For file tools we additionally relay each raw delta as a
  // `tool_call_update` carrying
  ...
  // Default off: identical to upstream unless FACTORY_STREAM_TOOL_INPUT=1.
  // When enabled, these notifications are composed with upstream refinements
  ...
  switch (event.type) {
    ...
    case "content_block_delta": {
      if (event.delta.type === "input_json_delta") {
        ...
        if (!streamedInput) return factoryUpdates;
        ...
        if (!input) return factoryUpdates;
        ...
        if (!update) return factoryUpdates;
        ...
        applyMessageId(update, options?.messageId);
        return [...factoryUpdates, { sessionId, update }];
      }
      return toAcpNotifications(
        [event.delta],
        "assistant",
